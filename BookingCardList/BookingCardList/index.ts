...
import { MapsProvider } from "./util/maps";

// fluentDesignLanguage carries the host app's modern theme (incl. overrides / dark mode).
interface FluentDesignContext {
    fluentDesignLanguage?: { tokenTheme?: Theme; isDarkTheme?: boolean };
}

export class BookingCardList implements ComponentFramework.ReactControl<IInputs, IOutputs> {
    ...
    public updateView(context: ComponentFramework.Context<IInputs>): React.ReactElement {
        ...
        const theme = (context as unknown as FluentDesignContext).fluentDesignLanguage?.tokenTheme;
        ...
        const props: BookingAppProps = {
            ...
            openItem: (id: string) => this.openItem(context, id),
            openUrl: (url: string) => context.navigation.openUrl(url),
            t: (key: string, fallback: string) => this.localize(context, key, fallback),
        };
    ...
    private customStatus(
        ...
        const workOrderSubStatusId = (
            context.parameters.customWorkOrderSubStatusId?.raw ?? ""
        ).trim();
        return {
            name,
